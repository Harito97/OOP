...
public class CountryArrayManager {
    ...
    public CountryData[] filterContinent(String continent) {
        /* TODO */
        int count = 0;
        for (CountryData x : countryDataArray){
            if (x.getContinent().equals(continent)){
                count++;
            }
        }
        CountryData[] continentArray = new CountryData[count];
        count = 0;
        for (CountryData x : countryDataArray){
            if (x.getContinent().equals(continent)){
                continentArray[count] = x;
                count++;
            }
        }
    ...
    public CountryData[] filterMostPopulousCountries(int howMany) {
        /* TODO */
        CountryData[] populationDes = sortDecreasingByPopulation();
        CountryData[] modPopulationContries = new CountryData[howMany];
        System.arraycopy(populationDes, 0, modPopulationContries, 0, howMany);
        return modPopulationContries;
    }

    // Lọc ra howMany nước có dân số ít nhất.
    public CountryData[] filterLeastPopulousCountries(int howMany) {
        /* TODO */
        CountryData[] populationDes = sortIncreasingByPopulation();
        CountryData[] minPopulationContries = new CountryData[howMany];
        System.arraycopy(populationDes, 0, minPopulationContries, 0, howMany);
        return minPopulationContries;
    }

    // Lọc ra howMany nước có diện tích lớn nhất.
    public CountryData[] filterLargestAreaCountries(int howMany) {
        /* TODO */
        CountryData[] areaDecreasing = sortDecreasingByArea();
        CountryData[] areaContries = new CountryData[howMany];
        System.arraycopy(areaDecreasing, 0, areaContries, 0, howMany);
        return areaContries;
    }
    ...
    public CountryData[] filterSmallestAreaCountries(int howMany) {
        /* TODO */
        CountryData[] populationDes = sortIncreasingByArea();
        CountryData[] modPopulationContries = new CountryData[howMany];
        System.arraycopy(populationDes, 0, modPopulationContries, 0, howMany);
        return modPopulationContries;
    }
    ...
    public CountryData[] filterHighestGdpCountries(int howMany) {
        /* TODO */
        CountryData[] populationDes = sortDecreasingByGdp();
        CountryData[] modPopulationContries = new CountryData[howMany];
        System.arraycopy(populationDes, 0, modPopulationContries, 0, howMany);
        return modPopulationContries;
    }
    ...
    public CountryData[] filterLowestGdpCountries(int howMany) {
        /* TODO */
        CountryData[] populationDes = sortIncreasingByGdp();
        CountryData[] modPopulationContries = new CountryData[howMany];
        System.arraycopy(populationDes, 0, modPopulationContries, 0, howMany);
        return modPopulationContries;
    }
